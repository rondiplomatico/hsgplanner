...
/**
 * The only Date class which should be used in PBK/Hadoop.
 * A Date is "YYYY-MM-HH". Usage: for all Dates which only needs year, month and day.
 * Ranges: from 0001-01-01 (no 0000-01-01 due to limits in Localdate) to 9999-12-31
 *
 * Wraps a java.sql.Date with useful methods.
 * Deprecated methods from java.sql.Date oder java.util.Date are no longer delegated.
 *
 * @author mattist
 * @since 28.04.2016
 */
public class HSGDate implements Serializable, Comparable<HSGDate>, KryoSerializable, KnownSizeEstimation {
    ...
    /**
     * New PBK Date from an ISO formatted date. * ISO format is e.g. '2011-12-03' or '2011-12-03+01:00'.
     * see {@link DateTimeFormatter.ISO_DATE}
     *
     * @param date
     *            the date
     * @return the PBK date
    ...
    /**
     * New PBK Date from an BASIC_ISO formatted date.
     ...
     *
     * @param numDay
     *            if positive number then returns the caller + param day (after)
     ...
     *
     * @param dayOfWeek
     *            the day of the week that should be returned
     * @return the first day of week
     */
    ...
    /**
     * Calculates the number of days between this <code>HSGDate</code> and the given other <code>HSGDate</code>.
     *
     * @param other
     *            the other <code>HSGDate</code>
     * @return number of days, negative if <code>other</code> is before <code>this</code>, and positive if
     *         <code>other</code> is after <code>this</code>.
     */
    ...
    /**
     * Gets the number of months between the month of this date and the specified date.
     * May return zero if there is no change of month between the two dates.
     *
     * @param other
     *            the other
    ...
    /**
     * Returns the number of week days (not saturday, sunday) between the two given dates (including both dates). No
     * distinction is made between work days and public holidays, however.
     *
     * This method is lenient with respect of the fields finer than day of week within the two given dates, i.e.
     *
     * @param toDate
     *            the end of the range of days within which to count the week days (must be the same day or after
     *            <code>toDate</code>).
     * @return the number of week days between both dates, including boundaries. Return -999 if one of the boundaries is
     *         null or if this date is after the toDate.
     */
    public int weekDayDifference(final HSGDate toDate) {
        ...
        while (fromLocalDate.compareTo(toLocalDate) <= 0) {
            if (fromLocalDate.getDayOfWeek() != DayOfWeek.SATURDAY && fromLocalDate.getDayOfWeek() != DayOfWeek.SUNDAY) {
                count++;
            }
